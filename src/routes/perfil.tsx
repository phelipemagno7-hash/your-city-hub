...
import { MapPin, Heart, Store, HelpCircle, Bell, ChevronRight, User, ShieldCheck, ArrowRight } from "lucide-react";
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="rounded-3xl border border-border bg-card p-6 shadow-card text-center space-y-4">
              <div className="grid size-20 mx-auto place-items-center rounded-3xl accent-soft text-2xl font-black text-primary shadow-sm">
                PM
              </div>
              <div>
                <h2 className="text-lg font-bold text-foreground">Phelipe Magno</h2>
                <p className="text-xs text-muted-foreground">phelipe@email.com</p>
                <span className="inline-flex items-center gap-1 mt-2 rounded-full bg-emerald-50 text-emerald-800 px-3 py-0.5 text-[11px] font-bold">
                  <ShieldCheck className="size-3.5" /> Cliente Verificado
                </span>
              </div>
            </div>
            ...
            <div className="rounded-3xl bg-gradient-to-r from-secondary to-primary p-6 text-primary-foreground shadow-lg flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="space-y-1 text-center sm:text-left">
                ...
                <p className="text-xs text-primary-foreground/85">
                  Venda seus produtos e divulgue seu trabalho para milhares de pessoas de Ipanema.
                </p>
              </div>
              <button className="shrink-0 inline-flex items-center gap-1.5 rounded-2xl bg-accent px-4 py-2.5 text-xs font-black text-secondary hover:bg-accent/90 transition-transform hover:scale-105 shadow-md">
